...
if [ "$ENABLE_SSL" = "true" ]; then
    ...
    IP=`hostname`
    sed -ri "s/IPADDR/$IP/g" $KAFKA_HOME/config/server.properties
    sed -ri "s/TRUSTSTORE_PASSWORD/$TRUSTSTORE_PASSWORD/g" $KAFKA_HOME/config/server.properties
    sed -ri "s/KEYSTORE_PASSWORD/$KEYSTORE_PASSWORD/g" $KAFKA_HOME/config/server.properties
    sed -ri "s/KEY_PASSWORD/$KEY_PASSWORD/g" $KAFKA_HOME/config/server.properties
    echo "SSL is enable on port 9093"
fi
